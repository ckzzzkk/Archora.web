-- Migration 015: Align quota_check RPC with tierLimits.ts
-- 005_quota_check_rpc.sql had Starter AI limit = 15.
-- tierLimits.ts and CLAUDE.md both say Starter = 10.
-- This migration corrects the authoritative DB value to 10.
...
BEGIN
  ...
  IF p_quota_type = 'ai_generation' THEN
    v_current := v_ai_used;
    v_limit := CASE v_tier
      WHEN 'starter'   THEN 10           -- was 15, corrected to match tierLimits.ts
