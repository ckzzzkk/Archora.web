-- Migration 032: Add 'pro' tier to quota_check RPC
-- Fixes: Pro users were rejected for all AI generation (hit ELSE → RETURN false)
-- Also adds missing 'architect' to ar_scan (was missing from 015_fix_quota_limits.sql)
...
BEGIN
  ...
  IF p_quota_type = 'ai_generation' THEN
    ...
    v_limit := CASE v_tier
      ...
      WHEN 'pro'       THEN 500   -- Pro tier: 500 AI generations/month
  ...
  ELSIF p_quota_type = 'ar_scan' THEN
    ...
    v_limit := CASE v_tier
      ...
      WHEN 'pro'       THEN 30    -- Pro: all AR modes (scan + place + measure)
      WHEN 'architect' THEN 2147483647
    END;
  ELSE
    RETURN false;
