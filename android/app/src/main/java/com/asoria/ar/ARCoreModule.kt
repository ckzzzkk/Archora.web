 ...
 *
 * Legacy bridge module (ReactContextBaseJavaModule). Under the New Architecture
 * it is reached from JS via the Native Module Interop Layer — the JS wrapper
 * (src/native/ARCoreModule.ts) resolves it by name through TurboModuleRegistry.
 */
class ARCoreModule(reactContext: ReactApplicationContext) : ReactContextBaseJavaModule(reactContext) {
    ...
    // Required so JS-side `new NativeEventEmitter(ARCoreModule)` does not warn/throw
    // under the New Architecture. The module emits via RCTDeviceEventEmitter, so
    // these are no-op book-keeping methods.

    @ReactMethod
    fun addListener(eventName: String) {
        // No-op: events are emitted globally via RCTDeviceEventEmitter.
    }
