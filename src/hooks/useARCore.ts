...
export function useARCore(): UseARCoreReturn {
  ...
  // Initialize - check capabilities + iOS session interruption listeners
  useEffect(() => {
    refreshCapabilities();

    // Listen for iOS ARKit session interruptions (e.g., phone call, app background)
    const subInterrupted = ARCoreModule.addSessionInterruptedListener(() => {
      if (isMounted.current) {
        setState(prev => ({ ...prev, isSessionActive: false, error: 'Session interrupted' }));
      }
    });
    const subResumed = ARCoreModule.addSessionResumedListener(() => {
      // Session auto-restarts in the native module delegate
      if (isMounted.current) {
        setState(prev => ({ ...prev, isSessionActive: true, error: null }));
      }
    });

    return () => {
      isMounted.current = false;
      stopSession(); // fire-and-forget cleanup
