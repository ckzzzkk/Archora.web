...
export function GardenSofaSet({ position, rotation, dimensions, color = '#8A9080', secondaryColor = '#5A6050', roughness = 0.9, metalness = 0.05, selected }: Props) {
...
export function SunLounger({ position, rotation, dimensions, color = '#D8C8A0', secondaryColor = '#808080', roughness = 0.7, metalness = 0.0, selected }: Props) {
  const c = selected ? '#4A90D9' : color;
  const { x: w, y: h, z: d } = dimensions;
  return (
    <group position={[position.x, position.y, position.z]} rotation={[rotation.x, rotation.y, rotation.z]}>
...
export function GardenDiningSet({ position, rotation, dimensions, color = '#6A5040', secondaryColor = '#8A7060', roughness = 0.8, metalness = 0.05, selected }: Props) {
  const c = selected ? '#4A90D9' : color;
  const { x: w, y: h, z: d } = dimensions;
  return (
    <group position={[position.x, position.y, position.z]} rotation={[rotation.x, rotation.y, rotation.z]}>
...
export function GardenBench({ position, rotation, dimensions, color = '#6A5040', secondaryColor = '#707070', roughness = 0.8, metalness = 0.05, selected }: Props) {
  const c = selected ? '#4A90D9' : color;
  const { x: w, y: h, z: d } = dimensions;
  return (
    <group position={[position.x, position.y, position.z]} rotation={[rotation.x, rotation.y, rotation.z]}>
...
export function SwingSet({ position, rotation, dimensions, color = '#8A6848', secondaryColor = '#C0A060', roughness = 0.7, metalness = 0.0, selected }: Props) {
  const c = selected ? '#4A90D9' : color;
  const { x: w, y: h, z: d } = dimensions;
  return (
    <group position={[position.x, position.y, position.z]} rotation={[rotation.x, rotation.y, rotation.z]}>
      ...
      {[-0.25, 0.25].map((t) => (
        <group key={t} position={[t * w, 0, 0]}>
          <mesh position={[0, h * 0.5, 0]}>
            ...
            <meshStandardMaterial color={secondaryColor} roughness={0.5} metalness={0} />
          </mesh>
          <mesh position={[0, h * 0.2, 0]} castShadow receiveShadow>
            <boxGeometry args={[0.35, 0.04, 0.25]} />
            <meshStandardMaterial color={secondaryColor} roughness={0.8} metalness={0} />
          </mesh>
        </group>
      ))}
      {selected && (
...
export function Trampoline({ position, rotation, dimensions, color = '#1A1A1A', secondaryColor = '#1A4080', roughness = 0.3, metalness = 0.5, selected }: Props) {
  const c = selected ? '#4A90D9' : color;
...
export function Sandpit({ position, rotation, dimensions, color = '#D8C090', secondaryColor = '#8A6040', roughness = 1.0, metalness = 0.0, selected }: Props) {
  const c = selected ? '#4A90D9' : color;
  const { x: w, y: h, z: d } = dimensions;
  return (
    <group position={[position.x, position.y, position.z]} rotation={[rotation.x, rotation.y, rotation.z]}>
...
export function Parasol({ position, rotation, dimensions, color = '#E8D0A0', secondaryColor = '#E0D8C8', roughness = 0.8, metalness = 0.0, selected }: Props) {
  const c = selected ? '#4A90D9' : color;
  const { x: w, y: h } = dimensions;
  return (
    <group position={[position.x, position.y, position.z]} rotation={[rotation.x, rotation.y, rotation.z]}>
      ...
      <mesh position={[0, h * 0.5, 0]} castShadow>
        ...
        <meshStandardMaterial color={secondaryColor} roughness={0.2} metalness={0.4} />
      </mesh>
      {/* Canopy */}
...
export function OutdoorKitchen({ position, rotation, dimensions, color = '#808080', secondaryColor = '#A8A8A8', roughness = 0.5, metalness = 0.3, selected }: Props) {
  const c = selected ? '#4A90D9' : color;
  const { x: w, y: h, z: d } = dimensions;
  return (
    <group position={[position.x, position.y, position.z]} rotation={[rotation.x, rotation.y, rotation.z]}>
...
export function Pergola({ position, rotation, dimensions, color = '#C0A870', secondaryColor = '#8A7850', roughness = 0.8, metalness = 0.0, selected }: Props) {
  const c = selected ? '#4A90D9' : color;
  const { x: w, y: h, z: d } = dimensions;
  return (
    <group position={[position.x, position.y, position.z]} rotation={[rotation.x, rotation.y, rotation.z]}>
...
export function GardenShed({ position, rotation, dimensions, color = '#6A7850', secondaryColor = '#4A3828', roughness = 0.9, metalness = 0.0, selected }: Props) {
  const c = selected ? '#4A90D9' : color;
  const { x: w, y: h, z: d } = dimensions;
  return (
    <group position={[position.x, position.y, position.z]} rotation={[rotation.x, rotation.y, rotation.z]}>
...
export function SwimmingPool({ position, rotation, dimensions, color = '#4090C0', secondaryColor = '#D8D0C0', roughness = 0.2, metalness = 0.05, selected }: Props) {
  const c = selected ? '#4A90D9' : color;
  const { x: w, y: h, z: d } = dimensions;
  return (
    <group position={[position.x, position.y, position.z]} rotation={[rotation.x, rotation.y, rotation.z]}>
      ...
      <mesh position={[0, -h * 0.4, 0]} receiveShadow>
        <boxGeometry args={[w, h * 0.8, d]} />
        <meshStandardMaterial color="#E8F0F0" roughness={roughness} metalness={metalness} />
      </mesh>
      {/* Water */}
      <mesh position={[0, 0, 0]} receiveShadow>
        <boxGeometry args={[w - 0.1, h * 0.05, d - 0.1]} />
        <meshStandardMaterial color={c} roughness={0.05} metalness={0.1} transparent opacity={0.85} />
      </mesh>
      {/* Pool surround */}
...
export function HotTub({ position, rotation, dimensions, color = '#6090A0', secondaryColor = '#484848', roughness = 0.3, metalness = 0.2, selected }: Props) {
  const c = selected ? '#4A90D9' : color;
  const { x: w, y: h } = dimensions;
  return (
    <group position={[position.x, position.y, position.z]} rotation={[rotation.x, rotation.y, rotation.z]}>
...
export function PlanterLarge({ position, rotation, dimensions, color = '#7A6050', secondaryColor = '#3A6030', roughness = 0.9, metalness = 0.0, selected }: Props) {
  const c = selected ? '#4A90D9' : color;
  const { x: w, y: h } = dimensions;
  return (
    <group position={[position.x, position.y, position.z]} rotation={[rotation.x, rotation.y, rotation.z]}>
...
export function PlanterSmall({ position, rotation, dimensions, color = '#7A6050', secondaryColor = '#4A7040', roughness = 0.9, metalness = 0.0, selected }: Props) {
  const c = selected ? '#4A90D9' : color;
  const { x: w, y: h } = dimensions;
  return (
    <group position={[position.x, position.y, position.z]} rotation={[rotation.x, rotation.y, rotation.z]}>
