...
export function FloorLamp({ position, rotation, dimensions, color = '#C0C0C0', secondaryColor = '#E8D8B0', roughness = 0.3, metalness = 0.8, selected, isOn = true }: Props) {
  ...
  return (
    <group position={[position.x, position.y, position.z]} rotation={[rotation.x, rotation.y, rotation.z]}>
      ...
      <mesh position={[0, shadeY, 0]} castShadow>
        ...
        <meshStandardMaterial color={selected ? '#4A90D9' : secondaryColor} roughness={0.8} metalness={0.0} side={2} />
      </mesh>
...
export function PendantLight({ position, rotation, dimensions, color = '#B0A030', secondaryColor = '#1A1A1A', roughness = 0.4, metalness = 0.6, selected, isOn = true }: Props) {
  const c = selected ? '#4A90D9' : color;
  const { x: w, y: h } = dimensions;
  ...
  return (
    <group position={[position.x, position.y, position.z]} rotation={[rotation.x, rotation.y, rotation.z]}>
      ...
      <mesh position={[0, h - 0.04 - cableLength * 0.5, 0]}>
        ...
        <meshStandardMaterial color={secondaryColor} roughness={0.9} metalness={0.0} />
      </mesh>

      {/* Shade (inverted cone) */}
      <mesh position={[0, shadeY, 0]} rotation={[Math.PI, 0, 0]} castShadow>
        <coneGeometry args={[shadeRadius, shadeHeight, 24, 1, true]} />
        <meshStandardMaterial color={c} roughness={roughness} metalness={metalness} side={2} />
      </mesh>
