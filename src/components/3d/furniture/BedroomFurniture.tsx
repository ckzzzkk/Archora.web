...
export function KingBed({ position, rotation, dimensions, color = '#D0C8B8', secondaryColor = '#4A3020', roughness = 0.6, metalness = 0.0, selected }: Props) {
...
export function PlatformBed({ position, rotation, dimensions, color = '#5A4030', secondaryColor = '#4A3020', roughness = 0.5, metalness = 0.05, selected }: Props) {
  const c = selected ? '#4A90D9' : color;
  const { x: w, y: h, z: d } = dimensions;
  return (
    <group position={[position.x, position.y, position.z]} rotation={[rotation.x, rotation.y, rotation.z]}>
      ...
      <mesh position={[0, h * 0.8, -d * 0.48]} castShadow>
        ...
        <meshStandardMaterial color={c} roughness={roughness * 0.8} metalness={metalness} />
      </mesh>
      {/* Pillows */}
      {[-0.3, 0.3].map((offset) => (
...
export function BunkBed({ position, rotation, dimensions, color = '#9A7850', secondaryColor = '#7A5830', roughness = 0.6, metalness = 0.0, selected }: Props) {
  const c = selected ? '#4A90D9' : color;
  const { x: w, y: h, z: d } = dimensions;
  const frameH = h * 0.1;
...
export function Crib({ position, rotation, dimensions, color = '#FFFFFF', secondaryColor = '#D8D0C0', roughness = 0.5, metalness = 0.0, selected }: Props) {
  const c = selected ? '#4A90D9' : color;
  const { x: w, y: h, z: d } = dimensions;
  return (
    <group position={[position.x, position.y, position.z]} rotation={[rotation.x, rotation.y, rotation.z]}>
...
export function ToddlerBed({ position, rotation, dimensions, color = '#FFFFFF', secondaryColor = '#D8C8B0', roughness = 0.5, metalness = 0.0, selected }: Props) {
  const c = selected ? '#4A90D9' : color;
  const { x: w, y: h, z: d } = dimensions;
  return (
    <group position={[position.x, position.y, position.z]} rotation={[rotation.x, rotation.y, rotation.z]}>
...
export function VanityDesk({ position, rotation, dimensions, color = '#D8D0C8', secondaryColor = '#C0A878', roughness = 0.4, metalness = 0.05, selected }: Props) {
  const c = selected ? '#4A90D9' : color;
  const { x: w, y: h, z: d } = dimensions;
  return (
    <group position={[position.x, position.y, position.z]} rotation={[rotation.x, rotation.y, rotation.z]}>
      ...
      <mesh position={[0, h * 0.72, -d * 0.155]}>
        ...
        <meshStandardMaterial color={secondaryColor} roughness={0.3} metalness={0.3} wireframe />
      </mesh>
      {/* Stool */}
...
export function ChangingTable({ position, rotation, dimensions, color = '#FFFFFF', secondaryColor = '#90C090', roughness = 0.5, metalness = 0.0, selected }: Props) {
  const c = selected ? '#4A90D9' : color;
  const { x: w, y: h, z: d } = dimensions;
  return (
    <group position={[position.x, position.y, position.z]} rotation={[rotation.x, rotation.y, rotation.z]}>
