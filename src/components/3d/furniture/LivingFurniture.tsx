...
export function BarUnit({ position, rotation, dimensions, color = '#8A6840', secondaryColor = '#D8C8A0', roughness = 0.6, metalness = 0.05, selected }: Props) {
  const c = selected ? '#4A90D9' : color;
  const { x: w, y: h, z: d } = dimensions;
  return (
    <group position={[position.x, position.y, position.z]} rotation={[rotation.x, rotation.y, rotation.z]}>
...
export function RoomDivider({ position, rotation, dimensions, color = '#B8A890', secondaryColor = '#5A4830', roughness = 0.8, metalness = 0.0, selected }: Props) {
  const c = selected ? '#4A90D9' : color;
  const { x: w, y: h, z: d } = dimensions;
  const panelCount = 3;
        ...
        return (
          <group key={i} position={[(-w / 2) + panelW * i + panelW / 2, 0, 0]} rotation={[0, angle, 0]}>
            ...
            <mesh>
              ...
              <meshStandardMaterial color={secondaryColor} roughness={0.5} metalness={0} wireframe />
            </mesh>
          </group>
        );
      })}
