...
export function Bookshelf({ position, rotation, dimensions, color = '#8B7355', secondaryColor = '#5A4830', roughness = 0.7, metalness = 0.0, selected = false }: BookshelfProps) {
        ...
        Array.from({ length: Math.floor((w - 0.08) / 0.06) }).map((_, bi) => {
          ...
          return (
            <mesh key={`book-${si}-${bi}`} position={[bookX, bookY, -d / 2 + d * 0.3]} castShadow>
              ...
              <meshStandardMaterial color={bookColor} roughness={0.9} metalness={0} />
            </mesh>
          );
        }),
      )}
