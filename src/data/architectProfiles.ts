...
export const ARCHITECT_CARDS: ArchitectCardData[] = ALL_ARCHITECTS.map((a) => ({
  id: a.id,
  name: a.name,
  era: a.era,
  philosophyTag: a.tagline.split(' ').slice(0, 2).join(' '),
  tagline: a.tagline,
  spatialSignature: a.spatialSignature.split('.')[0] + '.',
  iconProjects: getIconProjects(a.id),
}));

function getIconProjects(architectId: string): string[] {
  const projects: Record<string, string[]> = {
    'frank-lloyd-wright': ['Fallingwater', 'Robie House', 'Taliesin'],
    'zaha-hadid': ['Heydar Aliyev Center', 'Guangzhou Opera', 'MAXXI'],
    'tadao-ando': ['Church of Light', 'Naoshima Museum', 'Water Temple'],
    'norman-foster': ['The Gherkin', 'Apple Park', 'Millau Viaduct'],
    'le-corbusier': ["Villa Savoye", 'Notre Dame', "Unite d'Habitation"],
    'peter-zumthor': ['Therme Vals', 'Kunsthaus Bregenz', 'Brotherus Chapel'],
    'bjarke-ingels': ['8 House', 'CopenHill', 'VIA 57 West'],
    'kengo-kuma': ['Nezu Museum', 'Japan Pavilion Expo', 'Stone Museum'],
    'santiago-calatrava': ['Turning Torso', 'World Trade Center', 'Milwaukee Art Museum'],
    'alain-carle': ['House in Morin-Heights', 'Maison de la Pointe', 'Residence des Hauteurs'],
    'louis-kahn': ['Salk Institute', 'Kimbell Art Museum', 'National Assembly'],
    'rem-koolhaas': ['Seattle Central Library', 'Casa da Música', 'CCTV Headquarters'],
  };
  return projects[architectId] ?? ['Icon Project A', 'Icon Project B', 'Icon Project C'];
}
