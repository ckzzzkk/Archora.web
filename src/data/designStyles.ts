...
// ---------------------------------------------------------------------------
// Furniture style variant presets — per-piece material overrides
// primary = main surface color; secondary = legs/frame/accent color
// ---------------------------------------------------------------------------

export interface FurnitureVariantPreset {
  primary: string;
  secondary: string;
  roughness: number;
  metalness: number;
...
export const FURNITURE_VARIANT_PRESETS: Record<string, FurnitureVariantPreset> = {
  default:      { primary: '#8A7A6A', secondary: '#2D2D2D', roughness: 0.7, metalness: 0.0 },
  modern:      { primary: '#1A1A1A', secondary: '#C0C8C8', roughness: 0.2, metalness: 0.7 },
  rustic:      { primary: '#6A4020', secondary: '#3A2010', roughness: 0.9, metalness: 0.0 },
  mid_century: { primary: '#6A3818', secondary: '#D04820', roughness: 0.6, metalness: 0.1 },
  industrial:  { primary: '#404040', secondary: '#8A5A3A', roughness: 0.8, metalness: 0.4 },
  luxury:      { primary: '#C8A830', secondary: '#1A3058', roughness: 0.2, metalness: 0.8 },
};

export function getFurnitureVariant(variant: string): FurnitureVariantPreset {
  return FURNITURE_VARIANT_PRESETS[variant] ?? FURNITURE_VARIANT_PRESETS.default;
}

// ---------------------------------------------------------------------------
// Furniture model variants — shape sub-types per furniture type
// modelVariant on FurniturePiece picks which variant renders
// Each variant maps to a label + sub-style accent color for that silhouette
// ---------------------------------------------------------------------------

export interface FurnitureModelVariant {
  id: string;          // 'standard' | 'classic' | 'modern' | etc.
  label: string;        // 'Standard' | 'Classic' | 'Modern' | etc.
  accentColor: string;  // secondary accent for this silhouette variant
}

export const FURNITURE_MODEL_VARIANTS: Record<string, FurnitureModelVariant[]> = {
  // Seating
  sofa: [
    { id: 'standard',  label: 'Standard',  accentColor: '#4A4030' },
    { id: 'classic',  label: 'Classic',   accentColor: '#6A5030' },
    { id: 'modern',   label: 'Modern',    accentColor: '#1A1A1A' },
    { id: 'sleeper',  label: 'Sleeper',   accentColor: '#2A2820' },
  ],
  chair: [
    { id: 'standard',  label: 'Dining',   accentColor: '#4A4030' },
    { id: 'lounge',    label: 'Lounge',    accentColor: '#5A4838' },
    { id: 'classic',   label: 'Classic',  accentColor: '#6A5030' },
    { id: 'modern',    label: 'Modern',   accentColor: '#1A1A1A' },
  ],
  dining_chair: [
    { id: 'standard',  label: 'Standard',  accentColor: '#4A4030' },
    { id: 'classic',    label: 'Classic',   accentColor: '#6A5030' },
    { id: 'modern',     label: 'Modern',    accentColor: '#2A2A2A' },
  ],
  bar_stool: [
    { id: 'standard',  label: 'Standard',  accentColor: '#4A4030' },
    { id: 'modern',    label: 'Modern',    accentColor: '#C0C8C8' },
    { id: 'classic',   label: 'Classic',   accentColor: '#6A5030' },
  ],
  // Beds
  bed: [
    { id: 'standard',  label: 'Standard',  accentColor: '#4A3020' },
    { id: 'classic',   label: 'Classic',   accentColor: '#5A3828' },
    { id: 'platform',  label: 'Platform',  accentColor: '#2A2018' },
  ],
  bed_double: [
    { id: 'standard', label: 'Standard', accentColor: '#4A3020' },
    { id: 'classic', label: 'Classic',  accentColor: '#5A3828' },
  ...
  platform_bed: [
    { id: 'standard', label: 'Standard', accentColor: '#2A2018' },
    { id: 'classic',  label: 'Classic',  accentColor: '#5A3828' },
  ],
  bunk_bed: [
    { id: 'standard', label: 'Standard', accentColor: '#4A3020' },
  ...
  crib: [
    { id: 'standard', label: 'Standard', accentColor: '#E8E0D0' },
    { id: 'classic',  label: 'Classic',  accentColor: '#C0B8A0' },
  ],
  toddler_bed: [
    { id: 'standard', label: 'Standard', accentColor: '#E8E0D0' },
    { id: 'classic',  label: 'Classic',  accentColor: '#C0B8A0' },
  ...
  coffee_table: [
    ...
    { id: 'classic',  label: 'Classic',   accentColor: '#6A5030' },
    { id: 'modern',   label: 'Modern',    accentColor: '#1A1A1A' },
    { id: 'glass',    label: 'Glass Top', accentColor: '#C0D8E8' },
  ],
  round_dining_table: [
    { id: 'standard', label: 'Standard', accentColor: '#4A3020' },
    { id: 'classic',  label: 'Classic',  accentColor: '#6A5030' },
  ...
  modular_shelving: [
    { id: 'standard', label: 'Standard', accentColor: '#C0B8A8' },
    { id: 'modern',   label: 'Modern',    accentColor: '#2A2A2A' },
  ],
  tv_stand: [
    { id: 'standard', label: 'Standard',  accentColor: '#2A2018' },
    { id: 'classic',   label: 'Classic',   accentColor: '#4A3020' },
    { id: 'modern',   label: 'Modern',    accentColor: '#1A1A1A' },
  ],
  tv_media_unit: [
    { id: 'standard', label: 'Standard',  accentColor: '#2A2018' },
  ...
  // Kitchen
  kitchen_counter: [
    { id: 'standard', label: 'Standard', accentColor: '#B0A898' },
    { id: 'classic',   label: 'Classic',  accentColor: '#8A7060' },
  ],
  kitchen_island: [
    { id: 'standard', label: 'Standard', accentColor: '#B0A898' },
    { id: 'classic',  label: 'Classic',  accentColor: '#8A7060' },
  ...
  // Bathroom
  bathroom_sink: [
    { id: 'standard', label: 'Standard', accentColor: '#D0D0D0' },
    { id: 'modern',   label: 'Modern',   accentColor: '#F0F0F0' },
    { id: 'classic',  label: 'Classic',  accentColor: '#C0B8A8' },
  ],
  toilet: [
    { id: 'standard', label: 'Standard', accentColor: '#E8E8E8' },
    { id: 'modern',   label: 'Modern',  accentColor: '#F5F5F5' },
  ],
  bathtub: [
    { id: 'standard', label: 'Standard',  accentColor: '#E8E8E8' },
    { id: 'classic',  label: 'Classic',   accentColor: '#C8B8A0' },
    { id: 'modern',   label: 'Modern',    accentColor: '#F0F0F0' },
  ],
  freestanding_bath: [
    { id: 'standard', label: 'Standard', accentColor: '#E8E8E8' },
  ...
  // Living / decor
  fireplace_unit: [
    { id: 'standard', label: 'Standard',  accentColor: '#606060' },
    { id: 'classic',  label: 'Classic',   accentColor: '#8A7060' },
    { id: 'modern',   label: 'Modern',    accentColor: '#1A1A1A' },
  ],
  electric_fireplace: [
    { id: 'standard', label: 'Standard', accentColor: '#2A2A2A' },
    { id: 'modern',   label: 'Modern',  accentColor: '#1A1A1A' },
  ],
  bar_unit: [
    { id: 'standard', label: 'Standard', accentColor: '#6A5030' },
    { id: 'classic',  label: 'Classic',  accentColor: '#8A6840' },
    { id: 'modern',   label: 'Modern',   accentColor: '#2A2A2A' },
  ],
  room_divider: [
    { id: 'standard', label: 'Standard', accentColor: '#8A7060' },
    { id: 'modern',   label: 'Modern',  accentColor: '#1A1A1A' },
  ],
  // Lighting
  floor_lamp: [
    { id: 'standard', label: 'Standard',  accentColor: '#C0C0C0' },
    { id: 'modern',   label: 'Modern',    accentColor: '#1A1A1A' },
    { id: 'classic',  label: 'Classic',  accentColor: '#6A5030' },
  ],
  pendant_light: [
    { id: 'standard', label: 'Standard',  accentColor: '#B0A030' },
    { id: 'modern',   label: 'Modern',   accentColor: '#C0C8C8' },
    { id: 'classic',  label: 'Classic',  accentColor: '#8A6840' },
  ],
  // Outdoor
  garden_sofa_set: [
    { id: 'standard', label: 'Standard', accentColor: '#5A6050' },
    { id: 'modern',   label: 'Modern',  accentColor: '#2A2A2A' },
  ],
  garden_dining_set: [
    { id: 'standard', label: 'Standard', accentColor: '#5A4030' },
    { id: 'classic',  label: 'Classic',  accentColor: '#4A3020' },
  ],
  garden_bench: [
    { id: 'standard', label: 'Standard',  accentColor: '#5A4030' },
  ...
  sun_lounger: [
    { id: 'standard', label: 'Standard', accentColor: '#C8B880' },
    { id: 'modern',   label: 'Modern',  accentColor: '#2A2A2A' },
  ],
  pergola: [
    { id: 'standard', label: 'Standard', accentColor: '#A09060' },
    { id: 'classic',  label: 'Classic',  accentColor: '#6A5030' },
  ],
  hot_tub: [
    { id: 'standard', label: 'Standard', accentColor: '#508090A0' },
    { id: 'modern',   label: 'Modern',  accentColor: '#3A3A3A' },
  ],
  swimming_pool: [
    { id: 'standard', label: 'Standard', accentColor: '#4090C0' },
    { id: 'modern',   label: 'Modern',  accentColor: '#3A80B0' },
  ],
  // Stairs
  spiral_staircase: [
    { id: 'standard', label: 'Standard',  accentColor: '#808080' },
    { id: 'classic',  label: 'Classic',   accentColor: '#6A5030' },
    { id: 'modern',   label: 'Modern',    accentColor: '#2A2A2A' },
  ],
  l_staircase: [
    { id: 'standard', label: 'Standard',  accentColor: '#A09080' },
    { id: 'classic',  label: 'Classic',   accentColor: '#6A5030' },
  ],
  // Hard landscaping
  garden_path: [
    { id: 'standard', label: 'Standard', accentColor: '#8A8070' },
    { id: 'modern',   label: 'Modern',  accentColor: '#6A6050' },
  ],
  deck_area: [
    { id: 'standard', label: 'Standard', accentColor: '#A08050' },
    { id: 'classic',  label: 'Classic',  accentColor: '#6A5030' },
  ],
};

export function getModelVariants(category: string): FurnitureModelVariant[] {
  return FURNITURE_MODEL_VARIANTS[category] ?? [{ id: 'standard', label: 'Standard', accentColor: '#4A4030' }];
}

export function getModelVariant(category: string, modelVariant?: string): FurnitureModelVariant {
  const variants = getModelVariants(category);
  return variants.find(v => v.id === modelVariant) ?? variants[0];
}
